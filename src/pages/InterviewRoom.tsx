...
import { useSilenceRecorder } from "@/hooks/useSilenceRecorder";

type AvatarType = "easy" | "medium" | "hard";
type LanguageType = "en" | "ar" | "fr" | "es";

type ChatMsg = { role: "ai" | "user"; text: string };
type InterviewPhase = "LOADING" | "AI_SPEAKING" | "USER_SPEAKING" | "AI_THINKING" | "ENDED";
...
const getInterviewQuestions = (avatarType: AvatarType, language: LanguageType): string[] => {
  const questions: Record<AvatarType, Record<LanguageType, string[]>> = {
    easy: {
      en: ["Tell me about yourself.", "Why are you interested in this position?", "What are your greatest strengths?", "Where do you see yourself in 5 years?"],
      ar: ["أخبرني عن نفسك.", "لماذا أنت مهتم بهذا المنصب؟", "ما هي أعظم نقاط قوتك؟", "أين ترى نفسك بعد 5 سنوات؟"],
      fr: ["Parlez-moi de vous.", "Pourquoi êtes-vous intéressé par ce poste ?", "Quelles sont vos plus grandes forces ?", "Où vous voyez-vous dans 5 ans ?"],
      es: ["Háblame de ti.", "¿Por qué te interesa este puesto?", "¿Cuáles son tus mayores fortalezas?", "¿Dónde te ves en 5 años?"],
    },
    medium: {
      en: ["Describe a challenging project you worked on.", "How do you handle conflict in a team?", "What is your approach to problem-solving?", "Tell me about a time you failed and what you learned.", "How do you prioritize tasks when everything is urgent?"],
      ar: ["صف مشروعًا صعبًا عملت عليه.", "كيف تتعامل مع الصراع في الفريق؟", "ما هو نهجك في حل المشكلات؟", "أخبرني عن وقت فشلت فيه وماذا تعلمت.", "كيف تحدد أولويات المهام عندما يكون كل شيء عاجلاً؟"],
      fr: ["Décrivez un projet difficile sur lequel vous avez travaillé.", "Comment gérez-vous un conflit dans une équipe ?", "Quelle est votre approche de résolution de problèmes ?", "Parlez-moi d’un échec et de ce que vous en avez appris.", "Comment priorisez-vous quand tout est urgent ?"],
      es: ["Describe un proyecto difícil en el que trabajaste.", "¿Cómo manejas un conflicto en un equipo?", "¿Cuál es tu enfoque para resolver problemas?", "Cuéntame de un fracaso y lo que aprendiste.", "¿Cómo priorizas cuando todo es urgente?"],
    },
    hard: {
      en: ["Walk me through a complex technical decision you made.", "How would you design a system to handle millions of users?", "Describe a situation where you had to influence without authority.", "What would you do if you disagreed with your manager on a critical decision?", "How do you stay current with industry trends and technologies?", "Tell me about a time when you had to make a decision with incomplete information."],
      ar: ["اشرح لي قرارًا تقنيًا معقدًا اتخذته.", "كيف ستصمم نظامًا للتعامل مع ملايين المستخدمين؟", "صف موقفًا كان عليك فيه التأثير بدون سلطة.", "ماذا ستفعل إذا اختلفت مع مديرك في قرار حاسم؟", "كيف تبقى على اطلاع دائم باتجاهات الصناعة والتقنيات؟", "أخبرني عن وقت كان عليك فيه اتخاذ قرار بمعلومات غير كاملة."],
      fr: ["Expliquez une décision technique complexe que vous avez prise.", "Comment concevriez-vous un système pour des millions d’utilisateurs ?", "Décrivez une situation où vous avez influencé sans autorité.", "Que faites-vous si vous n’êtes pas d’accord avec votre manager sur une décision critique ?", "Comment restez-vous à jour sur les tendances et technologies ?", "Parlez-moi d’une décision prise avec des informations incomplètes."],
      es: ["Explica una decisión técnica compleja que tomaste.", "¿Cómo diseñarías un sistema para millones de usuarios?", "Describe una situación donde influiste sin autoridad.", "¿Qué harías si no estás de acuerdo con tu manager en una decisión crítica?", "¿Cómo te mantienes al día con tendencias y tecnologías?", "Cuéntame de una decisión con información incompleta."],
    },
  };
...
const getAvatarDuration = (avatarType: AvatarType): number => {
  const durations: Record<AvatarType, number> = { easy: 5 * 60, medium: 10 * 60, hard: 15 * 60 };
...
export default function InterviewRoom() {
  ...
  // Chat + “free STT” transcript (for Phase 1)
...
const resetSilenceTimer = () => {
  ...
  const recorder = useSilenceRecorder(stream, {
    ...
    silenceDurationMs: 1700,
  });

  // Prevent double-firing when recorder stops
...
// stop listening
  ...
  const beginAiTurn = useCallback(() => {
    ...
    // Phase 1: simulate speaking time (later TTS will control this)
    const speakDuration = Math.random() * 2000 + 2500;
    ...
    setTimeout(() => {
      ...
      // After AI finishes “speaking”, user turn starts automatically
      beginUserTurn();
    }, speakDuration);
  }, [beginUserTurn, recorder, stopListening]);
  ...
  const handleFinishAnswer = useCallback(async () => {
    ...
    if (nextIndex < questions.length) {
      ...
      // Start AI speaking automatically
      setTimeout(() => beginAiTurn(), 600);
    } else {
      handleEndInterview();
    }
  }, [
          ...
          <Card className="bg-card border-border">
            <CardContent className="p-4">
              <div className="relative aspect-video rounded-lg overflow-hidden">
                ...
                {phase === "USER_SPEAKING" && (
                  <div className="absolute top-4 right-4">
                    <Badge className="bg-green-600">Listening…</Badge>
                  </div>
                )}
              </div>
            </CardContent>
        ...
        <Card className="bg-gradient-to-r from-blue-900 to-purple-900 border-blue-700">
          <CardContent className="p-6">
            ...
            <div className="space-y-3 max-h-[360px] overflow-y-auto pr-2">
                ...
                <div key={i} className={`flex ${m.role === "ai" ? "justify-start" : "justify-end"}`}>
                  <div
                    className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                      m.role === "ai" ? "bg-white/15 text-white" : "bg-blue-600 text-white"
                    }`}
                  >
                    {m.text}
                  </div>
              ...
              {liveTranscript && phase === "USER_SPEAKING" && (
            ...
            <div className="flex items-center justify-between mt-6">
              ...
              {/* Optional visual mic meter (nice for realism) */}
              <div className="text-xs text-gray-300">
                {phase === "USER_SPEAKING" ? `Mic level: ${recorder.level.toFixed(3)}` : null}
              </div>
            </div>
          </CardContent>
        </Card>
